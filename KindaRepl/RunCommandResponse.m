...
@implementation RunCommandResponse
...
{
	if ([requestURL.path isEqualToString:@"/run"])
	{
		return YES;
	}
...
- (void)startResponse
{
    NSString *command = [url.query stringByRemovingPercentEncoding];
	NSData *fileData =
		[[[RunCommand sharedRunCommand] run:command] dataUsingEncoding:NSUTF8StringEncoding];
